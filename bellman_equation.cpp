...
void updateEnv(std::tuple<int, int> &state,std::vector<std::tuple<int, int>> &HOLE )
{

    int a = std::get<0>(state);
    int b = std::get<1>(state);

    int h01 = std::get<0>(HOLE[0]);
    int h02 = std::get<1>(HOLE[0]);

    int h11 = std::get<0>(HOLE[1]);
    int h12 = std::get<1>(HOLE[1]);

    int h21 = std::get<0>(HOLE[2]);
    int h22 = std::get<1>(HOLE[2]);

    std::cout << "-------------------------" << std::endl;
 
    MatrixX4c env;
    env << '_','_','_','_','_','_','_','_','_','_','_','_','_','_','_','_';
    env(a, b) = '0';
    env(h01,h02) = 'X';
    env(h11,h12) = 'X';
    env(h21,h22) = 'X';
...
std::vector<std::tuple<int, int>> landRocket(std::vector<std::map<std::string, double>> &qtable, std::tuple<int,int> &START, double &EPSILON, std::tuple<int, int> &TERMINAL, std::vector<std::tuple<int, int>> &HOLE ){
...
int main()
{
    ...
    double GAMMA = 0.9;
    double ALPHA = 0.05;
    std::tuple<int, int> TERMINAL = {3, 2};
    ...
    std::vector<std::tuple<int, int>> transitions = landRocket(qtable, START, EPSILON, TERMINAL, HOLE );
